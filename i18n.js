...
(() => {
  const pairs = {
    ...
    "runner.unsupportedAudio": ["Audio could not start. Enable sound and try again, or choose position-only or arithmetic n-back.", "Audio konnte nicht starten. Aktiviere Ton und versuche es erneut oder wähle Positions- oder Rechen-n-back."],
    ...
    "runner.audioNote": ["Turn sound on before starting. Speech uses only browser voices marked as local (localService) that match the selected language. If no matching local voice is available, the task uses distinct tones instead. Speech and tone stimulus sets are tracked separately; do not compare their scores directly.", "Schalte den Ton vor dem Start ein. Sprachausgabe nutzt ausschließlich vom Browser als lokal markierte Stimmen (localService), die zur gewählten Sprache passen. Ohne passende lokale Stimme nutzt die Aufgabe stattdessen unterscheidbare Töne. Sprach- und Tonreizsätze werden separat geführt; vergleiche ihre Werte nicht direkt."],
    ...
    "audio.failed": ["Audio playback failed during the task. The session is invalid; check sound and restart.", "Die Audiowiedergabe ist während der Aufgabe fehlgeschlagen. Die Sitzung ist ungültig. Prüfe den Ton und starte neu."],
    ...
    "task.dual-nback.desc": ["Track positions and sounds n steps back, separately or together; an arithmetic variant adds a fixed operand.", "Verfolge Positionen und Töne n Schritte zurück, einzeln oder gemeinsam; die Rechenvariante addiert einen festen Wert."],
    "task.dual-nback.instructions": [
      "First complete 8 mandatory practice trials; they do not count toward the main test. Compare each active stream with exactly n items earlier. Respond only to a match; press both buttons if both streams match. Do not respond during the first n items or to nonmatches, including repeats n−1 or n+1 steps back. Arithmetic: a match means the current number equals the number n steps back plus the configured operand.",
      "Absolviere zuerst 8 verpflichtende Übungsdurchgänge; sie zählen nicht zum Haupttest. Vergleiche jeden aktiven Reizstrom mit genau n Reizen zuvor. Antworte nur bei Übereinstimmung; drücke beide Tasten, wenn beide Ströme passen. Antworte nicht während der ersten n Reize oder bei Nichtübereinstimmung, auch nicht bei Wiederholungen n−1 oder n+1 Schritte zurück. Rechenvariante: Die aktuelle Zahl muss der Zahl n Schritte zuvor plus dem eingestellten Wert entsprechen."
    ],
    "task.dual-nback.keys": ["A = position match; L = audio match; both for both matches. Single arithmetic variant: A = arithmetic match. No match: press nothing.", "A = Positionsübereinstimmung; L = Audioübereinstimmung; beide für beide Treffer. Einzelne Rechenvariante: A = Rechenübereinstimmung. Keine Übereinstimmung: nichts drücken."],
